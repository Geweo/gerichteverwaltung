...
$containerStatus = docker ps --filter "name=ernaehrbar-ollama" --format "{{.Status}}"
...
$result = docker exec ernaehrbar-ollama ollama pull llama3.2 2>&1
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  docker exec -it ernaehrbar-ollama sh" -ForegroundColor Gray
